...
- 仅内部网络可访问的共享 PostgreSQL 与每日逻辑备份入口
...
共享 PostgreSQL 位于 `infrastructure/postgres/`，并由该栈创建 `internal: true` 的 `postgres-net`。需要数据库的应用把该网络声明为 external；PostgreSQL 只加入 `postgres-net`，不加入 `traefik-net`，也不发布 `5432`。不维护独立的空网络 Compose 项目。

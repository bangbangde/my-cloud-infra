...
- 供同机应用通过内部网络访问、供运维人员通过 SSH 隧道访问的共享 PostgreSQL，以及每日逻辑备份入口
- S3 API 仅供 Docker 内部访问、公开对象只读分发的单节点 Garage
...
共享 PostgreSQL 位于 `infrastructure/postgres/`，并由该栈创建 `internal: true` 的 `postgres-net`。需要数据库的应用把该网络声明为 external；PostgreSQL 只加入 `postgres-net`，不加入 `traefik-net`。宿主机端口仅绑定 `127.0.0.1:5432`，用于通过 SSH 隧道进行远程运维，不对公网监听。不维护独立的空网络 Compose 项目。

共享 Garage 位于 `infrastructure/garage/`，并创建 `internal: true` 的 `garage-net`。应用后端通过 `http://garage:3900` 使用 S3 API；Garage 同时加入 `traefik-net`，但 Traefik 只把 3902 的只读 Web endpoint 路由到公开媒体域名。S3、RPC 和管理端口均不发布到宿主机。
...
- PostgreSQL 镜像、持久化目录、内部网络和非公网暴露边界
